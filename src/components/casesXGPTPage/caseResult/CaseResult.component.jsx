...
import { CaseResultList, CaseResultMain } from './CaseResult.styles';
  ...
  return (
    <Separator>
      <CaseResultMain>
        ...
        <CasePageParagraph>{t(`main.homepage.singleCase.${currentCase.name}.resParagraph`)}</CasePageParagraph>
        <CaseResultList>
          {currentCase.resultList
            ? currentCase.resultList.map((el) => {
                return (
                  <li key={el}>
                    <img
                      src={liArrow}
                      alt=">"
                    />
                    <CasePageParagraph>{t(`main.homepage.singleCase.${currentCase.name}.resList.${el}`)}</CasePageParagraph>
                  </li>
                );
              })
            : null}
